...
fun main() = runBlocking{
    ...
    val modelRunnerBaseUrl = "http://localhost:11434"  // adjust to your local model's API base
    val modelName = "llama3"  // or the model you pulled / use

    val llmClient = OpenAILLMClient(
        settings = OpenAIClientSettings(baseUrl = modelRunnerBaseUrl),
        apiKey = System.getenv("OPENAI_API_KEY") ?: ""  // may be ignored or required depending model runner
    )
...
/*    val executor = simpleOpenAIExecutor(
    ...
    val transport = McpToolRegistryProvider.defaultSseTransport("http://localhost:8080")  // adjust to your MCP URL
